...
function todayScrollLeft() {
  ...
  // Pin midnight flush against the left edge of the *visible* plot — i.e. just
  // to the right of the fixed y-axis overlay (AXIS_W wide), so today starts hard
  // against the edge and everything before midnight is scrolled off.
  const inset = AXIS_W; // = overlay width (122px)
...
// to restore the horizontal scroll to. The scrollable width only exists after
// the async SVG renders, so opt out of scroll restoration and re-assert midnight
// across two frames so it wins over any late restore.
